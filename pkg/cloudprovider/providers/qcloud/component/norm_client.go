...
func NormCreateLB(namespace string, serviceName string, serviceUuid string, lBName string, lBType string, lbDomainPrefix string, subnetid int) error {
	c := NewNormClient()
	rsp := &NormNoRsp{}
	err := c.DoRequest(NORM_CREATE_LB, NormCreateLbReq{
		Namespace:   namespace,
