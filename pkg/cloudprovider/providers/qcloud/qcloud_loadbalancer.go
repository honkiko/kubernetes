...
func (self *QCloud) GetLoadBalancer(clusterName string, apiService *v1.Service) (status *v1.LoadBalancerStatus, exists bool, err error) {
	glog.V(0).Infof("garyyu-test GetLoadBalancer")
...
func (self *QCloud) ensureLBLister(clusterName string, apiService *v1.Service, hosts []string) error {
	glog.V(0).Infof("Ethan Inside ensureLBLister")
...
func (self *QCloud) ensureLBCreate(clusterName string, apiService *v1.Service, hosts []string) error {
	glog.V(0).Infof("Ethan Inside ensureHost")
	...
	//check that if there is a need for internal lb
	annotations := apiService.ObjectMeta.Annotations
	subnetID, isLBInternal := annotations[AnnoServiceLBInternalSubnetID]

	var iSubnetId int = 0
	//如果现在有的lb和要创建的lb类型不同，就删除现有的lb然后创建新的lb
	toCreateLBType := LB_CREATE_TYPE_OPEN
	if isLBInternal && len(subnetID) > 0 {
		toCreateLBType = LB_CREATE_TYPE_INTERNAL
		iSubnetId, err = getSubnetIdInt(subnetID)
		if err != nil {
			return err
		}
	}
	...
	lbName = fmt.Sprintf("ccs_%s_%s", cluster_id, lbName)
	...
	if toCreateLBType == LB_CREATE_TYPE_INTERNAL {
		err = norm.NormCreateLB(apiService.Namespace, apiService.Name, string(apiService.GetUID()), lbName, LB_CREATE_TYPE_INTERNAL, lbDomainPrefix, iSubnetId)
	} else {
		//default is the open LB
		err = norm.NormCreateLB(apiService.Namespace, apiService.Name, string(apiService.GetUID()), lbName, LB_CREATE_TYPE_OPEN, lbDomainPrefix, 0)
	}
...
func (self *QCloud) ensureLoadBalancer(clusterName string, apiService *v1.Service, hosts []*v1.Node) (*v1.LoadBalancerStatus, error) {
	glog.V(0).Infof("Ethan Inside ensureLoadBalancer")
...
func (self *QCloud) EnsureLoadBalancer(clusterName string, apiService *v1.Service, hosts []*v1.Node) (*v1.LoadBalancerStatus, error) {
	...
	if e != nil {
		glog.V(0).Infof("Ethan's let's see the return :%v", e.Error())
	} else {
