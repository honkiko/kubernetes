...
func initCheck() error {
	if ipsetPath == "" {
		...
		if err != nil {
			return errors.New(fmt.Sprintf("Error checking ipset version, assuming version at least 6.0.0: %v", err))
...
func (s *IPSet) Refresh(entries []string) error {
	...
	for _, entry := range entries {
		...
		if err != nil {
			errors.New(fmt.Sprintf("error adding entry %s to set %s: %v (%s)", entry, tempName, err, out))
		}
	}
